...
function RequiredMoisture() {
    let RequiredMoistureElement = document.getElementById('p2');
    ...
    if (NewRequiredMoisture > 100 || NewRequiredMoisture < 0) {
        ...
        RequiredMoistureElement.textContent = NewRequiredMoisture + "%";
    }
}

function WateringPeriod() {
    let WateringPeriodElement = document.getElementById('p3');
    let NewWateringPeriod = window.prompt("How often do you want your plant to be watered (DD/HH/MM)?");
    WateringPeriodElement.textContent = NewWateringPeriod.slice(0, 2) + " Days " + NewWateringPeriod.slice(3, 5) + " Hours " + NewWateringPeriod.slice(6, 8) + " Minutes";
}
